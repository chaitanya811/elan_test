...
#  site : http://paxcel.net/   http://boxupp.com/
 ...
 class tomcat		
			...
			$source_path 	= hiera('tomcat::source_path',	'/etc/puppet/modules/tomcat/files' ),
			...
			$check_tomcat 	= hiera('tomcat::check_tomcat',	'/opt/check_tomcat.txt'),
                        $env	= hiera('tomcat::check_env', 'PRODUCTION'),
                        $firewall	= hiera('firewall::service'),
			) {
	...
	case $platform {
    	...
    	case $java_version {
        	'8': {
            		$javaDownloadURI = "http://download.oracle.com/otn-pub/java/jdk/8u131-b11/d54c1d3a095b4ff2b6607d096fa80163/jdk-8u131-linux-${plat_filename}.tar.gz"
        ...
        exec { 'get_jdk_installer':
                ...
                unless => "ls -l $java_home/bin/java",
            	timeout => 600,
        ...
        exec { 'extract_jdk':
            	cwd     => "${java_dir}/",
            	command => "tar -zxvf ${installerFilename}",
            	creates => $java_home,
            	require => Exec['get_jdk_installer'],
        }
    }
	...
	file {"$tomcat_path":
        ...
        exec {"tomcat_source_path":
             command => 'mkdir -p /etc/puppet/modules/tomcat/files',
             }
			
        file {"/etc/puppet/modules/tomcat/files/${tomcat_file}":
                ensure  => present,
                owner   => root,
                group   => root,
                alias => apache-tomcat-file,
                source => "puppet:///modules/tomcat/${tomcat_file}",
                require => Exec["tomcat_source_path"],
             }
	...
	exec { "extract_tomcat":
		...
		require => File["$tomcat_path","apache-tomcat-file"];
		}	
	 ...
	 service {"${firewall}":
         ...
         file {"/usr/tomcat/${tomcat_version}/conf/server.xml":
                alias => serverxml,
                require => Exec["extract_tomcat"],
              }

         file {"/usr/tomcat/${tomcat_version}/conf/tomcat-users.xml":
         ...
         service {"tomcat":
                ...
                subscribe => File["serverxml", "usersxml"],
                }
         ...
         file {"/etc/httpd/conf.d/tomcat.conf":
                ...
                require => [ Package["httpd"], File["serverxml"] ]
              }
